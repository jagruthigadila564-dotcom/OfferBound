...
export default function Dashboard() {
  ...
  const handleAnalyze = async () => {
    ...
    setIsAnalyzing(true);
    try {
      // 1. Upload the resume — backend returns the saved resume's id
      const uploaded = await api.uploadResume(userId, selectedFile);

      // 2. Run the AI analysis against the pasted job description
      const analysis = await api.analyzeResume(uploaded.id, jobDescription);

      // 3. Hand the result to the Results page. Wouter has no route state,
      // so we stash it in sessionStorage and Results reads it on mount.
      ...
      <div className="relative max-w-5xl mx-auto px-6 py-12">
        ...
        >
          ...
          <div className="space-y-8">
            {/* Upload Section */}
            <div>
              <label className="block text-sm font-semibold mb-3">Your Resume</label>
            ...
            {error && (
              <p className="text-sm text-destructive text-center" data-testid="text-analyze-error">
                {error}
              </p>
            )}
